...
# ----------------- استعلام المصروفات الدراسية -----------------
@app.get("/get_fees")
def get_fees(national_id: str):
    session = requests.Session()
    
    # تنظيف الرقم القومي واستخراج الأرقام فقط (14 رقم)
    clean_id = "".join(re.findall(r'\d+', str(national_id)))
    if not clean_id:
        return {"success": False, "error": "رقم قومي غير صالح"}

    try:
        # 1. طلب الصفحة الأولى لجلب مفاتيح الـ ViewState وعناصر النموذج
        res_get = session.get(FEES_URL, headers=HEADERS, timeout=30)
        ...
        # استخراج اسم حقل الإدخال واسم زر الإرسال ديناميكياً
        text_inputs = soup_get.find_all("input", {"type": "text"})
        nat_input_name = "txt_nat_id"
        if text_inputs:
            nat_input_name = text_inputs[0].get("name", "txt_nat_id")

        submit_btn = soup_get.find("input", {"type": "submit"})
        btn_name = submit_btn.get("name", "btn_search") if submit_btn else "btn_search"
        btn_val = submit_btn.get("value", "بحث") if submit_btn else "بحث"
        ...
        res_post = session.post(FEES_URL, data=payload, headers=post_headers, timeout=30)
        soup = BeautifulSoup(res_post.text, "html.parser")

        # 3. فحص جدول المصروفات في صفحة الرد
